  ...
  const fetchWithRetry = async (retryCount = 0): Promise<void> => {
    try {
      ...
      const response = await fetch(
        'https://lastfm-last-played.biancarosa.com.br/rocky_shi/latest-song',
        {
        ...
        <div className="flex min-h-0 flex-shrink-0 flex-col justify-end">
          <div className="mr-8 flex flex-col">
            ...
            <span className="text-muted-foreground line-clamp-1 text-xs">
              <span className="text-muted-foreground">演唱者：</span>{' '}
              {artist['#text']}
            </span>
            <span className="text-muted-foreground line-clamp-1 text-xs">
              <span className="text-muted-foreground">专辑：</span> {album['#text']}
            </span>
          </div>
        </div>
      </div>
