...
<Callout title="为什么 `trieNode` 的设计不需要存储当前节点所表示的字符?" variant="note" defaultOpen={false}>

核心原因：字符信息已隐含在父节点中。

`trieNode` 结构体不需要存储当前节点对应的字符，因为**字符信息已经被隐含地编码在父节点的 `nexts` 数组索引中了**。

当你访问某个节点时，你总是通过以下方式到达它：

```go
move = move.nexts[ch-'a']  // 通过索引 (ch-'a') 访问子节点
```

这里 `ch-'a'` 这个索引值本身就包含了字符信息：
...
- 📖 父节点就像一本通讯录
- 🔤 `nexts[0]` 存储的是所有名字以 `'a'` 开头的联系人信息
- 📍 你通过翻到第 0 页（索引），就已经知道这一页是关于 `'a'` 的了

无需在联系人卡片上再写一遍 `'a'` 这个字母。

在 `Insert` 和 `Search` 方法中，你会发现从不需要"读取"节点本身的字符：

```go
func (t *Trie) search(target string) *trieNode {
    move := t.root
    for _, ch := range target {
        // 通过 ch-'a' 这个索引本身就知道了字符是什么
...
整个过程中，我们用 `ch` 这个循环变量来追踪字符，从不需要从节点中读取字符信息。

不存储字符还有额外的好处：**节省内存**。每个节点少存一个 `byte` 或 `rune` 字段，在大规模数据结构中会累积为可观的空间节省。
</Callout>
