---
title: '使用 Go 实现字典树（Trie）数据结构'
description: '本文介绍如何使用 Go 语言实现字典树（Trie）数据结构，并展示其在字符串存储和检索中的应用。'
date: 2025-12-07
tags: ['Go']
authors: ['rocky']
draft: false
...
前缀树 Trie，又称为字典树或者单词查找树，是一种广泛应用于文本词频统计场景的树状存储结构。
...
回顾其结构特征，我们可以看出，Trie 的优势是当存储拥有公共前缀的内容时，可以在很大程度上节省空间提高节点利用率。同时由于这种公共前缀的设计方式，也赋予了 Trie 能够支持前缀频率统计以及前缀模糊匹配的功能。
...
![](./assets/v2-5e01491485b8f050b6b8e415e05dc650_1440w.jpg)
...
- 每经过一个节点（包括根节点），都将其 `passCnt` 计数器加一。
...
到这里为止，我就把前缀树 Trie 的实现流程全部介绍完了。针对上述源码实现，如果其中存在纰漏或者大家有更优雅的实现方式，欢迎提出探讨。
...
关于 Radix Tree 的应用，其中一个很经典的工程案例就是 Gin 框架中针对路由模块的设计实现。

Gin 框架是 Go 语言中最流行的 Web 框架之一，开源地址：[https://github.com/gin-gonic/gin](https://github.com/gin-gonic/gin)

![](./assets/v2-eb7b0d75db615fbf3ed37240b29d735e_1440w.jpg)

在 Gin 框架中，使用 Radix Tree 作为路由树的数据结构，建立了请求路径 pattern 和路径处理函数链 handlers 之间的映射关系。之所以选用 Radix Tree 而非 HashMap，其原因在于：

- 在路由匹配时，除了需要根据请求路径进行精确匹配之外，还有着模糊匹配的诉求。比如请求路径末尾 `'/'` 符号的增减、通配符 `'*'` 的使用等，这种非单点匹配的操作不适合使用 Map，而 Radix Tree 则能够满足诉求
- 各路由长度相比于数量而言更加有限，使用 Radix Tree 实现的话可以有更好的性能表现

Radix Tree 具体实现是比较具有难度的，主要在于随着单词的插入和删除，需要对 Radix Tree 结构进行动态调整，涉及到对父子节点的合并和拆分。本文第 4 章内容中对 Radix Tree 的源码还原就是借鉴了 Gin 框架中的实现，更多有关 Gin 的内容，大家可以阅读我之前发表的文章——《解析 Gin 框架底层原理》。
