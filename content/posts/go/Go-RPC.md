...
```go
func main() {
  	// RegisterName 类似于 Register，但使用提供的名称为类型注册而不是接收者的具体类型
...
其中 `rpc.Register` 方法会将类型实例中所有满足 RPC 规则的方法注册为 RPC 函数，所有注册的方法会放在 `HelloService` 服务空间之下。然后我们建立一个唯一的 TCP 连接，并且通过 `rpc.ServeConn` 函数在该 TCP 连接上为对方提供 RPC 服务。
...
func main() {
    ...
    var reply string
    err = client.Call("HelloService.Hello", "iris", &reply)
    if err != nil {
        log.Fatal(err)
    }
...
首先是通过 `rpc.Dial` 拨号 RPC 服务，然后通过 `client.Call` 调用具体的 RPC 方法。在调用 `client.Call` 时，第一个参数是用点号连接的 RPC 服务名字和方法名字，第二和第三个参数分别是传递给远程函数的参数及远程函数的返回值。

## 打造更安全的 RPC 接口

在涉及 RPC 的应用中，作为开发人员一般划分为三类角色：首先是服务端实现 RPC 方法/服务的开发人员，其次是客户端调用 RPC 方法的人员，最后也是最重要的是制定服务端和客户端 RPC 接口规范的设计人员。在前面的例子中我们为了简化将以上几种角色的工作全部放到了一起，虽然看似实现简单，但是不利于后期的维护和工作的切割。
